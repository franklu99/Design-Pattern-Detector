...
X = data.drop(['pattern', 'url', 'project_name', 'class_name',
...
def adaboost_tree():
    return AdaBoostClassifier(base_estimator=RandomForestClassifier(n_estimators=1000), n_estimators=20,
                              learning_rate=0.5)


def adaboost_logistic():
    return AdaBoostClassifier(
        base_estimator=LogisticRegression(solver='lbfgs', class_weight="balanced", penalty="l2", multi_class="auto",
...
def logistic():
    return make_pipeline(StandardScaler(),
                         LogisticRegression(solver='saga', class_weight="balanced", penalty="l1", multi_class="auto",
                                            n_jobs=-1))
...
def svm():
    return make_pipeline(StandardScaler(),
                         SVC(class_weight="balanced", gamma='scale', C=1000.0, kernel="rbf", max_iter=100000,
                             probability=True))
...
confusion_matrix_output_file = results_directory + "/confusion_matrix_%s.csv" % algorithm
if not os.path.exists(results_directory):
    os.makedirs(results_directory)
with open(confusion_matrix_output_file, "w") as f:
    ordered_patterns = []
...
if precision_recall_single_plot:
    df1 = pd.DataFrame(
        {"Design Pattern": label_lookup, "value": precision, "Metric": ["Precision"] * len(label_lookup)})
    ...
    sns.barplot(label_lookup, precision, ax=ax1)
    # Uncomment below line to add title to plot
    # ax1.set_title("Precision = %.2f"%(precision_final))
    ax1.set_xlabel("Design Pattern")
    ax1.set_ylabel("Precision")
    plt.xticks(rotation=45)
    plt.savefig("results/%s Precision Scores" % (algorithm))
    plt.show()

    f, (ax2) = plt.subplots(1, 1, figsize=(6, 7))
    sns.barplot(label_lookup, recall, ax=ax2)
    # Uncomment below line to add title to plot
    # ax2.set_title("Recall = %.2f"%(recall_final))
    ax2.set_xlabel("Design Pattern")
    ax2.set_ylabel("Recall")
    plt.xticks(rotation=45, ha="right")

    plt.savefig("results/%s Recall Scores" % (algorithm))
    plt.show()
